"""
MEXC OHLCV Data Source
Provides clean interface for fetching MEXC OHLCV data using CCXT
"""
...
class MEXCOHLCVDataSource:
    """Fetches OHLCV data from MEXC using CCXT - uses SWAP (perpetuals) for MASSIVE liquidity"""
    
    def __init__(self, api_key: Optional[str] = None, api_secret: Optional[str] = None, market_type: str = 'swap'):
        """Initialize MEXC CCXT client for swap (perpetuals) - 175,000x higher volume than spot!"""
        ...
        if not self.api_key or not self.api_secret:
            try:
                config = load_config()
                self.api_key = self.api_key or config.get('mexc_api_key')
                self.api_secret = self.api_secret or config.get('mexc_api_secret')
            except Exception as e:
                logger.warning(f'Could not load MEXC API credentials from central config loader: {e}')        
        ...
        self.exchange = ccxt.mexc(config)  # type: ignore
        logger.info(f"Initialized MEXCOHLCVDataSource for {market_type}")
    ...
    def fetch_historical_data(
        ...
        Args:
            ...
            limit: Number of candles to fetch (max 2000 for MEXC)
        ...
        try:
            ...
            logger.debug(f"Fetching MEXC data for {symbol} ({timeframe}), limit={limit}")
            
            # Fetch OHLCV data using CCXT
            ohlcv = self.exchange.fetch_ohlcv(
                symbol=symbol,
                timeframe=timeframe,
                limit=min(limit, 2000),  # MEXC max is 2000
                since=since
            )
            ...
            logger.error(f"Error fetching MEXC data for {symbol} ({timeframe}): {e}")
            return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    
    def get_available_timeframes(self) -> list:
        """Get list of supported timeframes for MEXC SWAP"""
        return ['1m', '5m', '15m', '30m', '1h', '4h', '8h', '1d', '1w', '1M']
    
    def validate_symbol(self, symbol: str) -> bool:
        """Check if symbol is available on MEXC"""
        try:
            # Convert symbol format if needed
            if '/' not in symbol:
                if self.market_type == 'swap':
