"""
Bybit OHLCV Data Source
Provides clean interface for fetching Bybit OHLCV data using CCXT
"""
...
class BybitOHLCVDataSource:
    """Fetches OHLCV data from Bybit using CCXT - uses SWAP (perpetuals) for high liquidity"""
    
    def __init__(self, api_key: Optional[str] = None, api_secret: Optional[str] = None, market_type: str = 'swap'):
        """Initialize Bybit CCXT client for swap (perpetuals) - 3x higher volume than spot"""
        ...
        if not self.api_key or not self.api_secret:
            try:
                config = load_config()
                self.api_key = self.api_key or config.get('bybit_api_key')
                self.api_secret = self.api_secret or config.get('bybit_api_secret')
            except Exception as e:
                logger.warning(f'Could not load Bybit API credentials from central config loader: {e}')
        ...
        logger.info(f"Initialized BybitOHLCVDataSource for {market_type}")
    ...
    def fetch_historical_data(
        ...
        Args:
            ...
            limit: Number of candles to fetch (max 1000 for Bybit)
        ...
        try:
            ...
            logger.debug(f"Fetching Bybit data for {symbol} ({timeframe}), limit={limit}")
            ...
            logger.error(f"Error fetching Bybit data for {symbol} ({timeframe}): {e}")
            return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    
    def get_available_timeframes(self) -> list:
        """Get list of supported timeframes for Bybit SWAP"""
